...
- Landing-page palette is deliberately different from the dashboard's cyan: dark green (`emerald-700`/`800`/`950`) as the brand/anchor color on a warm cream background (`--color-cream` / `bg-cream`, defined in `index.css`). The dashboard and `GoalForm` stay cyan/stone — don't propagate the green there without being asked.
