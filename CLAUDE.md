...
- `src/pages/seoCalculators/` — standalone, single-purpose calculator pages that live at their own URLs for search traffic (`/vacation-savings-calculator`, `/how-much-to-save-per-paycheck`, etc.). `registry.js` is the source of truth (pure data — no JSX, imported directly by `scripts/prerender.js` in Node): one entry per page drives its `<head>` (title/meta/canonical/OG + FAQ JSON-LD, built in `src/entry-server.jsx`), its visible copy (`SeoCalculatorPage.jsx`), and its sitemap URL. `StandaloneCalculator.jsx` is the embedded tool — same `calculations.js` math as the dashboard, no localStorage, one headline number with the breakdown behind a toggle. These are still not a router: `src/main.jsx` checks `location.pathname` against the registry at mount and hydrates the matching page; the build writes one `dist/<slug>/index.html` per entry and regenerates `dist/sitemap.xml`. `index.html` carries `<!-- seo:head:start/end -->` markers that `scripts/prerender.js` swaps per page. `vercel.json` (`cleanUrls`) serves `/<slug>` from `dist/<slug>/index.html`. To add a page: add a registry entry — nothing else.
