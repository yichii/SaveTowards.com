...
- Currently a header + Hero + a thin trust-message ticker (`src/components/landing/Hero.jsx`, `HeroDemo.jsx`, `useHeroDemo.js`, `TrustTicker.jsx`): headline (with an animated underline on "exactly"), subhead, "Start a goal" CTA, and a Fill-style demo card built on the real `FillIcon`/`calculateSavingsPlan` code.
- The demo card autoplays through three example goals (Trip to Japan / New car / Wedding fund) on load, animating each fill from 0 to its target percent, until the user first touches the slider — at that point autoplay stops permanently (no snap-back) and the slider takes over. This state machine lives in `useHeroDemo.js`; `HeroDemo.jsx` is purely presentational, driven by props. `usePrefersReducedMotion` (`src/hooks/usePrefersReducedMotion.js`) disables autoplay and the ticker's scroll for users who request it.
