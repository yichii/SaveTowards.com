...
export function LivePlanPreview({ targetAmount, amountSaved, targetDate, payFrequency, takeHomePay }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <span className="text-sm font-medium text-emerald-900">Your savings plan</span>
      <div className="rounded-lg bg-emerald-50 p-4 text-sm">
        {!validAmount || !validDate ? (
          <p className="text-emerald-900/60">Fill in the amount and date to see your savings plan.</p>
        ) : (
          <PlanBreakdown
...
function IncomeShareNote({ plan }) {
  ...
  return (
    <div className="mt-2 text-emerald-900/60">
...
function PlanBreakdown({ plan }) {
  if (plan.status === 'met') {
    return <p className="text-emerald-900/60">You've already reached this goal — nice work!</p>
  }

  if (plan.status === 'overdue') {
    return <p className="text-emerald-900/60">This date has already passed — pick a date in the future to see a plan.</p>
  }

  if (plan.status === 'due-today') {
    return <p className="text-emerald-900/60">Target date is today — you'd need {currency.format(plan.amountRemaining)} today.</p>
  }
    ...
    <>
      <div className="grid grid-cols-2 gap-3 @lg:grid-cols-4">
        {plan.rows.filter((row) => row.visible).map((row) => (
          <div key={row.key}>
            <p className="text-emerald-900/60">{row.label}</p>
