...
export function SharedGoalCard({ name, category, emoji, includeAmounts, targetAmount, amountSaved, daysToSave }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50/50 p-6 text-center shadow-sm">
      <FillIcon emoji={glyph} label={cat?.label} percent={100} size={72} />
      <p className="flex items-center gap-1.5 font-heading text-xl font-semibold text-emerald-700">
        <PartyPopper size={20} className="shrink-0" />
        {label} — goal reached!
      </p>
      {includeAmounts ? (
        <div className="flex flex-col gap-0.5">
          <p className="text-sm text-emerald-900/70">{currency.format(amountSaved)} saved</p>
          {daysToSave != null && (
            <p className="text-xs text-emerald-900/55">Saved {formatDuration(daysToSave)}</p>
          )}
        </div>
      ) : (
        <p className="text-sm text-emerald-900/70">100% saved</p>
      )}
    </div>
  )
}
